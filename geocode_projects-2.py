...
Zweck: Ermittelt lat/lng Koordinaten für alle Städte aus der Excel-Datei
und schreibt eine fertige projects.csv im Schema-Format (Semikolon, Dezimalkomma).

Voraussetzungen:
    pip install openpyxl geopy

Ausführung:
    python3 geocode_projects.py

Ausgabe: projects.csv im selben Ordner, bereit zum Verschieben nach data/projects.csv
"""
...
# ── Stadtname/Bundesland-Korrekturen ──────────────────────────────────────
CITY_CORRECTIONS = {
...
# Manuelle Überschreibungen für Standorte die Nominatim nicht findet.
# Format: "Stadtname|Bundesland": (lat, lng)
# Koordinaten über https://www.openstreetmap.org suchen, Ort anklicken, Werte aus URL ablesen.
MANUAL_OVERRIDES = {
    # "Hifstätten Raab|Steiermark": (47.0, 15.8),
}
...
def geocode_all(unique_locations):
    # SSL-Verifikation deaktivieren (Workaround für Homebrew Python / macOS)
    ...
    geolocator = Nominatim(user_agent="mission_intelligence_ffg_map_v1")

    results = {}
    failed = []

    # Checkpoint laden wenn vorhanden (Fortsetzung nach Abbruch)
    ...
    for i, (city, bundesland) in enumerate(unique_locations):
        ...
        query = f"{city}, {bundesland}, Austria"
        try:
            loc = geolocator.geocode(query, timeout=10)
            if loc:
                results[key] = [round(loc.latitude, 4), round(loc.longitude, 4)]
                print(f"[{i+1}/{len(unique_locations)}] {query} -> OK ({loc.latitude:.4f}, {loc.longitude:.4f})")
            else:
                failed.append((city, bundesland))
                print(f"[{i+1}/{len(unique_locations)}] {query} -> NOT FOUND")
        except Exception as e:
            failed.append((city, bundesland))
            print(f"[{i+1}/{len(unique_locations)}] {query} -> ERROR: {e}")

        # Nach jeder Anfrage Checkpoint speichern
        with open(CHECKPOINT_FILE, 'w') as f:
            json.dump(results, f, ensure_ascii=False)
        time.sleep(1.1)
...
def main():
    ...
    rows_written = 0
    rows_skipped = 0
    photo_counter = {}  # Zählt Foto-Pins pro Mission für video_pin_order
    ...
    with open(OUTPUT_FILE, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        for row, row_type in all_rows:
            ...
            if key not in coords:
                print(f"⚠️  Übersprungen (keine Koordinate): {row[name_idx] or row[id_idx]} ({city}, {bundesland})")
                rows_skipped += 1
                continue
            ...
            if row_type == 'pin':
                ...
                if video_type == 'youtube' and videolink:
                    ...
                    video_pin_order = 99  # Video-Pin immer zuletzt
                else:
    ...
    print(f"   Stichprobe der Koordinaten prüfen, besonders bei kleinen/unbekannten Orten.")
    if os.path.exists(CHECKPOINT_FILE):
